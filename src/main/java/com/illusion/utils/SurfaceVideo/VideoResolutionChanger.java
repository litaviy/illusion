package com.illusion.utils.SurfaceVideo;
...
public class VideoResolutionChanger {
    ...
    private static String OUTPUT_VIDEO_MIME_TYPE = "video/avc"; // H.264 Advanced Video Coding
    //    private static final int OUTPUT_VIDEO_BIT_RATE = 2048 * 1024;
    private static int OUTPUT_VIDEO_FRAME_RATE = 30;// 15fps
    private static int OUTPUT_VIDEO_BIT_RATE = 1500000; // 2Mbps
    private static int OUTPUT_VIDEO_IFRAME_INTERVAL = 10; // 10 seconds between I-frames
    private static int OUTPUT_VIDEO_COLOR_FORMAT =
            MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface;

    private static String OUTPUT_AUDIO_MIME_TYPE = "audio/mp4a-latm";// Advanced Audio Coding
    private static int OUTPUT_AUDIO_CHANNEL_COUNT = 2;// Must match the input stream.
    private static int OUTPUT_AUDIO_BIT_RATE = 128 * 1024;
    private static int OUTPUT_AUDIO_AAC_PROFILE =
...
//    private int mHeight = -1;
     ...
     *
     * @param inputFile input file video
     * @param outFile   output directory to change the file
     * @param listener  Run with complete or error
     * @return true - good, false - error problem
     * @throws Throwable
     */
    public boolean changeResolution(File inputFile, File outFile, ResultListener listener)
            throws Throwable {
...
//        final File outFile = new File(outDir, fileName);
        ...
        try {
            ...
            listener.onFailureResult(t.getMessage());
            return false;
        }
     ...
     * Compressing Video file
     *
     * @param inputFilePath input file video path (getAbsolutePath())
     * @param outFile       output directory to change the file
     * @param listener      Run with complete or error
     * @return true - good, false - error problem
     * @throws Throwable
     */
    public boolean changeResolution(String inputFilePath, File outFile, ResultListener listener)
            throws Throwable {
        mInputFile = inputFilePath;
...
//        final File outFile = new File(outDir, fileName);
    ...
    /**
     * Set param Compression
     *
     * @param name  Key from MediaFormat
     * @param param value
     */
    public void setFlag(String name, int param) {
        switch (name) {
            case MediaFormat.KEY_FRAME_RATE:
                OUTPUT_VIDEO_FRAME_RATE = param;
                break;
            case MediaFormat.KEY_BIT_RATE:
                OUTPUT_VIDEO_BIT_RATE = param;
                break;
            case MediaFormat.KEY_I_FRAME_INTERVAL:
                OUTPUT_VIDEO_IFRAME_INTERVAL = param;
                break;
            case MediaFormat.KEY_COLOR_FORMAT:
                OUTPUT_VIDEO_COLOR_FORMAT = param;
                break;
            case MediaFormat.KEY_WIDTH:
                mWidth = param;
                break;
            case MediaFormat.KEY_HEIGHT:
                mHeight = param;
                break;
            default:
                break;
     ...
     */
    public void setMimeType(String mime, int width, int height) {
        OUTPUT_VIDEO_MIME_TYPE = mime;
        mWidth = width;
        mHeight = height;
    }
